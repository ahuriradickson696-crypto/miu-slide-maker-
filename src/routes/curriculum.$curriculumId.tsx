...
function CurriculumDetailInner() {
  ...
  const slots = useMemo(
    () => (structure ? flattenSlots(structure) : []),
    [structure],
  );
  ...
  useEffect(() => {
    ...
    getCurriculum({ data: { id: curriculumId } })
      .then((c) => {
        ...
        setHasSourceFile(c.hasSourceFile);
      })
      .catch(
        (e) =>
          !cancelled &&
  ...
  useEffect(() => {
    ...
    getSemesterNotes({
      data: {
        curriculumId,
        yearLabel: activeSlot.year,
        semesterLabel: activeSlot.semester,
      },
    })
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const message =
        e instanceof Error
          ? e.message
          : "Couldn't generate notes for this semester.";
  ...
  async function handleDownloadOriginal() {
    ...
    try {
      const { url } = await getCurriculumFileUrl({
        data: { id: curriculumId },
      });
      window.open(url, "_blank", "noopener,noreferrer");
    } catch (e) {
      toast.error(
        e instanceof Error ? e.message : "Couldn't get the original file",
      );
    } finally {
    ...
    <div className="min-h-screen bg-[#F7F8F5]">
      ...
      <div className="print-hide border-b bg-white/90 backdrop-blur sticky top-0 z-10">
        <div className="mx-auto max-w-5xl px-4 sm:px-6 py-3 flex items-center gap-3">
          ...
          <a
            href={`/curriculum/${curriculumId}/structure`}
          ...
          >
            ...
            <span className="hidden sm:inline">Structure builder</span>
          </a>
          ...
          {hasSourceFile && (
            <button
              ...
              className="inline-flex items-center gap-1.5 rounded-full border px-2.5 sm:px-3 py-1.5 text-xs font-medium hover:border-primary hover:text-primary transition disabled:opacity-60"
            >
              {downloadingOriginal ? (
          ...
          {notes && (
            <>
              ...
              <button
                type="button"
                onClick={() =>
                  downloadCurriculumSemesterMarkdown(programName, notes)
                }
          ...
          <div className="print-hide rounded-xl border bg-white mb-6 overflow-hidden">
            ...
            >
              ...
              <span className="ml-auto text-xs text-muted-foreground">
                {slots.length} semester{slots.length === 1 ? "" : "s"}
              </span>
            </button>
            <AnimatePresence>
              {outlineOpen && (
                      ...
                      <div key={y.year}>
                        ...
                        {y.semesters.map((s) => {
                          const idx = slots.findIndex(
                            (sl) =>
                              sl.year === y.year && sl.semester === s.semester,
                          );
                          return (
                            <button
                              key={s.semester}
                            ...
                            >
                              ...
                              <span className="text-muted-foreground">
                                {" "}
                                — {s.courseUnits.length} unit
                                {s.courseUnits.length === 1 ? "" : "s"} (
                                {s.courseUnits.reduce(
                                  (n, u) => n + u.topics.length,
                                  0,
                                )}{" "}
                                topics)
                              </span>
                ...
                {notesLoading ? (
                  ...
                  <div className="print-hide text-center py-8">
                    ...
                    <p className="text-sm font-medium">
                      Notes for {activeSlot.label} haven't been generated yet
                    </p>
                    <p className="mt-1 text-xs text-muted-foreground max-w-md mx-auto">
                      Every topic in this semester gets a full definition, key
                      principles, a real-world application, and a takeaway —
                      nothing skipped, no placeholders.
                    </p>
                    {genError && (
                    ...
                    >
                      {generating ? (
                        ...
                        <>
                          <Sparkles className="h-4 w-4" /> Generate notes for{" "}
                          {activeSlot.label}
                        </>
                      )}
                    </motion.button>
                  ...
                  <>
                    ...
                    <div className="print-hide border-t pt-5 flex flex-col items-center gap-3 text-center">
                      <p className="text-sm text-muted-foreground">
                        ...
                        {!isLastSlot &&
                          " — shall I proceed to the next semester?"}
                      </p>
...
function TopicBlock({
  ...
  topic: SemesterNotes["topics"][number];
  index: number;
}) {
  return (
    <section className="pb-5 border-b last:border-b-0 last:pb-0">
      <p className="text-[11px] font-semibold uppercase tracking-wide text-accent mb-1">
        {[topic.courseUnitCode, topic.courseUnitTitle]
          .filter(Boolean)
          .join(" — ")}
      </p>
      <h3
        className="text-base sm:text-lg font-semibold text-primary mb-3"
        style={{ fontFamily: "var(--font-display)" }}
      >
