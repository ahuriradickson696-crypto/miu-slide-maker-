...
function StudioPage() {
  return (
    <ErrorBoundary>
      <AuthGate>
        {(username, onLogout) => (
          <StudioPageInner username={username} onLogout={onLogout} />
        )}
      </AuthGate>
    </ErrorBoundary>
  );
...
// ========== Login gate ==========
// Nobody sees the studio (or can call any of its server functions — those
// are guarded separately by authMiddleware) without logging in first.
function AuthGate({
  children,
}: {
  children: (username: string, onLogout: () => void) => ReactNode;
}) {
  ...
  async function handleLogout() {
    try {
      ...
      // Even if the server call fails, drop back to the login screen —
      // the cookie will just get overwritten on the next successful login.
    }
    setStatus("out");
    setUsername("");
  }
...
function LoginScreen({
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      {/* Brand panel — left, hidden on small screens, like standard
          institutional SSO screens (Okta / Azure AD / Google Workspace). */}
      <div
      ...
      >
        ...
        <div className="text-xs text-white/60">
          © {new Date().getFullYear()} Metropolitan International University
          · Kampala · Mbarara · Kisoro
        </div>
      </div>
          ...
          <form onSubmit={handleSubmit} noValidate className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full rounded-md border border-input bg-background px-3 py-2.5 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition"
            ...
            <button
              type="submit"
              disabled={submitting}
              className="w-full inline-flex items-center justify-center gap-2 rounded-md bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90 disabled:opacity-60 transition"
            >
              {submitting ? (
                <>
        ...
        <section className="rounded-2xl bg-card border p-6 slide-shadow h-fit sticky top-6">
          ...
          {deck && (
            <p className="mt-2 text-[11px] text-muted-foreground text-center">
              Includes full lecturer speaker notes in the PowerPoint's Notes pane for every slide.
            </p>
          )}
        </section>
    ...
    >
      ...
      {spec.speakerNotes && (
        <div className="border-t">
          ...
          >
            <span>{notesOpen ? "Hide" : "Show"} speaker notes</span>
            <span className="text-[10px] text-primary">
              {notesOpen ? "▲" : "▼"}
            </span>
          </button>
          {notesOpen && (
            <p className="px-3 pb-3 text-xs leading-relaxed text-slate-600 whitespace-pre-wrap">
              {spec.speakerNotes}
            </p>
          )}
        </div>
