...
function StudioPageInner() {
  ...
  // configured a shared GROQ_API_KEY server-side — either way, a
  ...
  useEffect(() => {
    ...
    try {
      setConfigBannerDismissed(
        sessionStorage.getItem("miu-config-banner-dismissed") === "1",
      );
    } catch {
  ...
  const googleClientId = (import.meta as any).env?.VITE_GOOGLE_CLIENT_ID as
    string | undefined;
  ...
  function updateAuthForm<K extends keyof typeof authForm>(
    key: K,
    value: (typeof authForm)[K],
  ) {
  ...
  // Account & Privacy — download my data / delete my account.
  ...
  async function handleExportMyData() {
    ...
    try {
      ...
      link.href = url;
      link.download = `miu-studio-my-data-${new Date().toISOString().slice(0, 10)}.json`;
      link.click();
  ...
  async function handleDeleteAccount() {
    ...
    try {
      ...
      // The account (and its session) no longer exists — return to Home,
      // which will show the signed-out state.
  ...
  // Live rate-limit countdown. When Groq's free tier (30 req/min, 14,400/day)
  ...
  async function handleBulkRegenerate() {
    ...
    if (failures === 0) toast.success(`Regenerated ${indices.length} slides`);
      ...
      toast.error(
        `Regenerated ${indices.length - failures} of ${indices.length} slides — some failed`,
      );
  }
  ...
  function toggleShare() {
    setShareOpen((v) => {
      ...
      if (next && savedDeckId) {
        getActiveShareLink({
          data: { resourceType: "deck", resourceId: savedDeckId },
        })
          .then((r) =>
            setShareLink(
              r.token ? `${window.location.origin}/shared/${r.token}` : null,
            ),
          )
  ...
  async function handleRevokeShareLink() {
    ...
    try {
      await revokeShareLink({ data: { token } });
      setShareLink(null);
  ...
  function toggleVersionHistory() {
    setVersionHistoryOpen((v) => {
      const next = !v;
      if (next && savedDeckId) refreshVersions();
      return next;
    });
  }
  ...
  async function refreshVersions() {
    ...
    try {
      const list = await listDeckVersions({ data: { deckId: savedDeckId } });
      setVersions(list);
    } catch {
      setVersions([]);
    } finally {
  ...
  async function handleSaveVersion() {
    if (!savedDeckId || !deck) return;
    setSavingVersion(true);
    try {
      const note =
        window.prompt(
          'Optional note for this version (e.g. "before restructuring"):',
          "",
        ) ?? "";
      const { versionNumber } = await saveDeckVersion({
        data: { deckId: savedDeckId, slides: deck.slides, note },
      });
      toast.success(`Saved as version ${versionNumber}`);
      refreshVersions();
    } catch (e) {
      toast.error(
        e instanceof Error ? e.message : "Couldn't save this version",
      );
    } finally {
  ...
  async function handleRestoreVersion(versionNumber: number) {
    ...
    try {
      const result = await restoreDeckVersion({
        data: { deckId: savedDeckId, versionNumber },
      });
      setDeck((d) => (d ? { ...d, slides: result.slides } : d));
      toast.success(`Restored version ${versionNumber}`);
      refreshVersions();
    } catch (e) {
      toast.error(
        e instanceof Error ? e.message : "Couldn't restore that version",
      );
    } finally {
  ...
  async function refreshHistory() {
    ...
    try {
      const result = await listDecks({
        data: { offset: 0, limit: HISTORY_PAGE_SIZE },
      });
  ...
  async function loadMoreHistory() {
    ...
    try {
      const result = await listDecks({
        data: { offset: history.length, limit: HISTORY_PAGE_SIZE },
      });
  ...
  async function handleLoadDeck(id: string) {
    ...
    try {
      ...
      console.error(e);
      toast.error(e instanceof Error ? e.message : "Couldn't load that deck");
    } finally {
  ...
  async function handleDeleteDeck(id: string, e: React.MouseEvent) {
    ...
    toast(
      `Deleted "${removed.topic || removed.suggestedFilename || "Untitled deck"}"`,
      {
        action: {
          label: "Undo",
          onClick: () => {
  ...
  // The key never touches a server other than the AI provider's — it's
  // kept in the browser only, so returning users don't have to paste it
  // every time. Managed centrally on the Settings page now.
  useEffect(() => {
  ...
  function buildGeneratePayload(outline?: { title: string; type: string }[]) {
    // In "paste" mode, the AI provider extracts topic/course identification
    ...
    return mode === "paste"
      ? {
          ...
          creditUnits: "",
          contactTime: "",
          targetPrograms: form.targetPrograms,
          outline,
        }
  ...
  async function handleReviewOutline() {
    if (
      ...
      return;
    if (!hasApiAccess) return toast.error("Add an API key in Settings first");
    ...
    try {
      ...
      console.error(e);
      const message =
        e instanceof Error ? e.message : "Couldn't build an outline";
  ...
  async function handleGenerate(
    pinnedOutline?: { title: string; type: string }[],
  ) {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      console.error(e);
      toast.error(
        e instanceof Error && e.message ? e.message : "PDF export failed",
      );
    } finally {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="mx-auto max-w-7xl px-3 sm:px-6 py-3 sm:py-4 flex items-center gap-2 sm:gap-3">
                    ...
                    {authMenuOpen && (
                      ...
                      >
                        ...
                        {isAdmin && (
                          ...
                          >
                            <ListChecks className="h-3.5 w-3.5" /> Usage
                            dashboard
                          </a>
                        )}
                        ...
                        <button
                          ...
                          className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left hover:bg-muted transition"
                        >
                          <FileDown className="h-3.5 w-3.5" /> Account & Privacy
                        </button>
                        <button
                          type="button"
                          role="menuitem"
              ...
              <div className="mx-auto max-w-7xl px-3 sm:px-6 py-2 flex items-start gap-2 text-xs">
                ...
                <div className="flex-1 min-w-0">
                  <span className="font-medium">
                    Some optional features aren't configured yet:{" "}
                  </span>
                  {[
                    !configStatus.database &&
                      "deck History (needs DATABASE_URL)",
                    !configStatus.session &&
                      "sign-in / accounts (needs SESSION_SECRET)",
                    configStatus.session &&
                      configStatus.passwordAuth &&
                      !configStatus.email &&
                      "password-reset emails (needs a Resend API key — reset links are logged server-side for now)",
                    !configStatus.redis &&
                      "distributed rate limiting (needs Upstash Redis)",
                  ]
                    .filter(Boolean)
                    .join(" · ")}
                  . Generating and downloading decks still works fine. See
                  DEPLOYMENT.md.
                </div>
                <button
                  type="button"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                {savedDeckId && (
                  <div className="rounded-lg border bg-muted/30 p-3">
                    <p className="text-xs font-semibold mb-2">Shareable link</p>
                    ...
                    {shareLink ? (
                      ...
                      <motion.button
                        ...
                        className="w-full inline-flex items-center justify-center gap-1.5 rounded-md border px-3 py-2 text-xs font-medium hover:border-primary hover:text-primary transition disabled:opacity-60"
                      >
                        {creatingShareLink ? (
                          <Loader2 className="h-3.5 w-3.5 animate-spin" />
                        ) : (
      ...
      {/* Version history — save a checkpoint, browse/restore past ones */}
      <AnimatePresence>
        {versionHistoryOpen && (
          <motion.div
            className="fixed inset-0 z-50 flex justify-end"
            ...
            >
              ...
              <div className="p-4 border-b">
                ...
                <p className="mt-1.5 text-[11px] text-muted-foreground leading-relaxed">
                  Checkpoints your current edits so you can always come back to
                  this exact state.
                </p>
              </div>
              <div className="flex-1 overflow-y-auto p-4 space-y-2">
                {loadingVersions ? (
                  ...
                  <p className="text-xs text-muted-foreground text-center py-6">
                    No saved versions yet — save one above.
                  </p>
                ) : (
                  versions.map((v) => (
                    <div
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-[10px] text-muted-foreground">
                    Applies to the slide preview and both PowerPoint/PDF
                    exports.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <p className="mt-1.5 text-[10px] text-muted-foreground">
                    Translates key labels only — this is an early preview, not
                    full coverage yet.
                  </p>
                </div>
                ...
                <div>
                  ...
                  {isStandalone ? (
                    ...
                    <motion.button
                      ...
                      className="w-full inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground hover:opacity-90 transition"
                    >
                      <Smartphone className="h-4 w-4" /> Install app on this
                      device
                    </motion.button>
                  ) : isIOS ? (
                    <p className="flex items-start gap-1.5 rounded-lg bg-muted/50 p-2.5 text-[11px] text-muted-foreground leading-relaxed">
                      ...
                      Tap the <Share2 className="inline h-3 w-3 -mt-0.5" />{" "}
                      Share button in Safari, then{" "}
                      <strong>"Add to Home Screen"</strong> — it'll open
                      full-screen like an app.
                    </p>
                  ) : (
                    <p className="text-[11px] text-muted-foreground leading-relaxed">
                      Open this site in Chrome on your phone to install it as an
                      app.
                    </p>
                  )}
                </div>
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Preferences are saved on this device and applied automatically
                  next time you visit.
                </p>
              </div>
            </motion.div>
                ...
                {authMode === "forgot" && forgotSent ? (
                  <div className="rounded-lg border bg-primary/5 p-4 text-center">
                    ...
                    <p className="mt-1 text-xs text-muted-foreground leading-relaxed">
                      ...
                      {authForm.email || "that address"}, we've sent a link to
                  ...
                  <form onSubmit={handleAuthSubmit} className="space-y-3">
                    ...
                    >
                      <input
                        ...
                        onChange={(e) =>
                          updateAuthForm("email", e.target.value)
                        }
              ...
              <div className="flex-1 overflow-y-auto p-4">
                ...
                >
                  <Field
                    ...
                    icon={<KeyRound className="h-3.5 w-3.5" />}
                    hint="Leave blank if you signed up with Google and haven't set a password yet."
                  >
                    <input
                      type="password"
        ...
        {accountPrivacyOpen && (
          ...
          >
            <motion.div
              ...
              variants={{ open: { opacity: 1 }, closed: { opacity: 0 } }}
              transition={{ duration: 0.2 }}
              onClick={() => !deletingAccount && setAccountPrivacyOpen(false)}
            />
            <motion.div
              role="dialog"
              aria-modal="true"
            ...
            >
              <div className="flex items-center justify-between border-b px-4 py-3">
                <h3 className="font-semibold text-sm">Account & Privacy</h3>
                <button
                  type="button"
                  onClick={() => setAccountPrivacyOpen(false)}
                  aria-label="Close"
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-6">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    Get a copy of your profile, decks, lecture notes, and
                    curricula as a single file.
                  </p>
                  <button
                    type="button"
                    onClick={handleExportMyData}
                    disabled={exportingData}
                    className="w-full inline-flex items-center justify-center gap-2 rounded-lg border px-4 py-2.5 text-sm font-medium hover:border-primary hover:text-primary transition disabled:opacity-60"
                  >
                    {exportingData ? (
                      <Loader2 className="h-4 w-4 animate-spin" />
                    ) : (
                ...
                <div className="border-t pt-5">
                  ...
                  <p className="text-xs text-muted-foreground mb-3">
                    ...
                    decks, notes, curricula, and uploaded files. This cannot be
                    undone.
                  </p>
                  {!deleteConfirming ? (
                    <button
                      ...
                      className="w-full inline-flex items-center justify-center gap-2 rounded-lg border border-destructive/40 px-4 py-2.5 text-sm font-medium text-destructive hover:bg-destructive/10 transition"
                    >
                    ...
                    <div className="space-y-2 rounded-lg border border-destructive/40 bg-destructive/5 p-3">
                      <p className="text-xs text-destructive">
                        Type <strong>DELETE</strong> to confirm — this is
                        permanent.
                      </p>
                      <input
                        type="text"
                        ...
                        placeholder="DELETE"
                        className="w-full rounded-lg border bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-destructive/40"
                      />
                      <div className="flex gap-2">
                        <button
                          ...
                          disabled={deletingAccount}
                          className="flex-1 rounded-lg border px-3 py-2 text-xs font-medium hover:bg-muted transition disabled:opacity-50"
                        >
                          Cancel
                        </button>
                        <button
                          ...
                          className="flex-1 inline-flex items-center justify-center gap-1.5 rounded-lg bg-destructive px-3 py-2 text-xs font-semibold text-destructive-foreground hover:opacity-90 disabled:opacity-50 transition"
                        >
                          {deletingAccount ? (
                            <Loader2 className="h-3.5 w-3.5 animate-spin" />
                          ) : (
      ...
      <main className="mx-auto max-w-7xl px-3 sm:px-6 py-5 sm:py-8 grid gap-6 sm:gap-8 lg:grid-cols-[380px_1fr]">
        ...
        >
          ...
          {/* API key status — managed centrally in Settings now, not pasted inline here. */}
          <div className="mb-5 rounded-xl border bg-gradient-to-br from-muted/60 to-muted/20 p-3.5">
            ...
            {apiKey.trim() ? (
              <p className="mt-1.5 flex items-center gap-1.5 text-[11px] text-primary">
                ...
                Using your saved key — your personal quota.{" "}
                <a
                  href="/settings"
              ...
              <p className="mt-1.5 text-[11px] text-muted-foreground leading-relaxed">
                Using the shared key for this deployment.{" "}
                <a
                  href="/settings"
                ...
                to use your personal quota instead.
              </p>
            ) : (
              <p className="mt-1.5 text-[11px] text-muted-foreground leading-relaxed">
                No key yet —{" "}
            ...
            <div className="space-y-3">
              {!form.topic.trim() && !deck && (
                ...
                >
                  <span className="flex items-center gap-1.5">
                    <Sparkles className="h-3.5 w-3.5" /> New here? Try a
                    filled-in example
                  </span>
              ...
              >
                <Input
                  ...
                  onChange={(v) => update("targetPrograms", v)}
                  placeholder="e.g. Civil, Mechanical, and Electrical Engineering"
                />
                <span className="mt-1 block text-[10px] text-muted-foreground">
                  For a course shared across multiple programs/departments —
                  contextualizes the overview slide.
                </span>
              </Field>
              <Field
          ...
          {cooldown && (
            <div className="mt-2 space-y-1.5 rounded-lg border border-amber-200 bg-amber-50 p-3">
              ...
              <p className="text-[11px] text-amber-700">
                The button unlocks itself automatically — no need to keep
                checking.
              </p>
            </div>
          )}
              ...
              >
                <div className="flex items-center justify-between gap-3 mb-1">
                  <h2 className="text-lg font-semibold truncate">
                    {outlineReview.detectedTopic}
                  </h2>
                  <span className="shrink-0 text-xs text-muted-foreground">
                    {outlineReview.outline.length} slides
                  </span>
                </div>
                <p className="text-xs text-muted-foreground mb-4">
                  Edit titles, reorder, add, or remove slides — then build the
                  full deck from this plan.
                </p>
                ...
                <div className="mt-4 flex gap-2">
                  ...
                  >
                    <Sparkles className="h-4 w-4" /> Build full deck from this
                    outline
                  </motion.button>
                </div>
              </motion.div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      ...
                      {!bulkMode && (
                        <>
                          {" "}
                          • drag{" "}
                          <GripVertical className="inline h-3 w-3 -mt-0.5" /> to
                          reorder
                        </>
                      )}
                    </p>
                  ...
                  {bulkMode && selectedSlides.size > 0 && (
                    ...
                    >
                      ...
                      <button
                        ...
                        disabled={
                          bulkBusy ||
                          deck.slides.length - selectedSlides.size < 2
                        }
      ...
      <ChatWidget
        contextLabel={deck ? `Slide deck: ${deck.topic}` : "MIU Slide Studio"}
        contextSummary={
          deck
            ? deck.slides
                .map((s) => slideToPlainText(s))
                .join("\n\n")
                .slice(0, 6000)
            : ""
        }
  ...
  return (
    ...
    >
      {Array.from({ length: count }).map((_, i) => {
        const role =
          i === 0 ? "title" : i === count - 1 ? "takeaway" : "content";
        return (
          <motion.div
            key={i}
            layout
...
function GeneratingLabel() {
  ...
  useEffect(() => {
    const id = setInterval(
      () => setI((v) => (v + 1) % GENERATING_STAGES.length),
      1800,
    );
  ...
  return (
    <div className="rounded-2xl border-2 border-dashed border-destructive/40 bg-destructive/5 p-10 text-center">
      ...
      <p className="mt-1 text-sm text-muted-foreground max-w-sm mx-auto">
        {message ||
          "Something went wrong talking to the AI service. Your brief is still filled in — try again."}
      </p>
      <motion.button
        type="button"
        onClick={onRetry}
        ...
        <div className="min-h-screen flex items-center justify-center bg-background px-6">
          <div className="max-w-md rounded-2xl border p-8 text-center slide-shadow">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              The page hit an unexpected error and couldn't continue. Your API
              key is still saved — reloading should get you back to a clean
              state.
            </p>
            <button
    ...
    <div className="print-slide group rounded-xl overflow-hidden border bg-card slide-shadow">
      ...
      >
        {!editing && (
          ...
          >
            <GripVertical
              className={`h-3.5 w-3.5 ${isTitle ? "text-white/90" : "text-slate-500"}`}
            />
          </div>
        )}
        <div
        ...
        {editing ? (
            ...
            {!isTitle && (
              <>
                ...
                <textarea
                  ...
                  onChange={(e) =>
                    setDraft((d) => ({ ...d, bulletsText: e.target.value }))
                  }
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center px-6 text-center">
            <img src={logo} alt="" className="h-16 w-16 object-contain mb-2" />
      ...
      <div className="print-hide px-3 py-2 flex items-center justify-between text-xs bg-muted/40">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            disabled={disabled}
            aria-label={
              editing ? `Save slide ${index + 1}` : `Edit slide ${index + 1}`
            }
            className="rounded p-1 text-muted-foreground hover:bg-muted hover:text-foreground transition disabled:opacity-30"
          >
          ...
          >
            <div className="flex items-center gap-1.5">
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && refineText.trim())
                    handleRegenerate(refineText.trim());
                }}
              ...
              <button
                type="button"
                onClick={() =>
                  refineText.trim() && handleRegenerate(refineText.trim())
                }
