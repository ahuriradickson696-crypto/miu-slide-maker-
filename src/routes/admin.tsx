...
type Tab = "overview" | "users" | "decks" | "notes" | "curricula" | "system";
      ...
      {authorized === false && (
        <div className="mx-auto max-w-md px-4 py-24 text-center">
          ...
          <p className="mt-3 text-xs text-muted-foreground">
            This page is restricted to admins. The designated first admin
            account is promoted automatically on sign-in, and can then add or
            remove other admins from the Users tab.
          </p>
          <a
            href="/"
    ...
    <div className="space-y-8">
      ...
      <div>
        <h2 className="text-sm font-semibold mb-3">
          Decks per day (last 14 days)
        </h2>
        <div className="rounded-xl border bg-card p-4">
          {stats.byDay.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              No decks generated in this window yet.
            </p>
          ) : (
...
function UsersTab() {
  ...
  async function load(reset: boolean) {
    ...
    try {
      const result = await adminListUsers({
        data: { offset: reset ? 0 : users.length, limit: 25, query },
      });
  ...
  async function toggleAdmin(u: AdminUser) {
    ...
    try {
      ...
      setUsers((prev) =>
        prev.map((x) => (x.id === u.id ? { ...x, isAdmin: !x.isAdmin } : x)),
      );
      toast.success(
        !u.isAdmin
      ...
      toast.error(
        e instanceof Error ? e.message : "Couldn't update admin status",
      );
    } finally {
      setTogglingId(null);
    }
  ...
  return (
    <div>
            ...
            <div key={u.id} className="flex items-center gap-3 p-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-muted-foreground truncate">
                  ...
                  {[u.hasGoogle && "Google", u.hasPassword && "Password"]
                    .filter(Boolean)
                    .join(" + ") || "—"}
                </p>
              </div>
      ...
      {hasMore && !loading && (
        <LoadMoreButton onClick={() => load(false)} loading={loadingMore} />
      )}
    </div>
  );
}
...
function DecksTab() {
  ...
  async function load(reset: boolean) {
    reset ? setLoading(true) : setLoadingMore(true);
    try {
      const result = await adminListDecks({
        data: { offset: reset ? 0 : decks.length, limit: 25, query },
      });
  ...
  return (
    <div>
      <SearchBar
        value={query}
        onChange={setQuery}
        placeholder="Search by topic or owner email…"
      />
      {loading ? (
        <Loader2 className="h-5 w-5 animate-spin text-primary" />
      ) : decks.length === 0 ? (
        ...
        <div className="rounded-xl border bg-card divide-y">
          {decks.map((d) => (
            <div key={d.id} className="flex items-center gap-3 p-3">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium truncate">
                  {d.topic || "Untitled deck"}
                </p>
                <p className="text-xs text-muted-foreground truncate">
                  {[d.courseCode, d.courseName].filter(Boolean).join(" • ") ||
                    "—"}{" "}
                  • {d.slideCount} slides • {d.ownerEmail || "no owner"} •{" "}
...
type AdminNotes = Awaited<
  ReturnType<typeof adminListLectureNotes>
>["notes"][number];
...
function NotesTab() {
  ...
  async function load(reset: boolean) {
    reset ? setLoading(true) : setLoadingMore(true);
    try {
      const result = await adminListLectureNotes({
        data: { offset: reset ? 0 : notes.length, limit: 25, query },
      });
      setNotes((prev) => (reset ? result.notes : [...prev, ...result.notes]));
      setHasMore(result.hasMore);
    } catch (e) {
      toast.error(
        e instanceof Error ? e.message : "Couldn't load lecture notes",
      );
    } finally {
      setLoading(false);
      setLoadingMore(false);
...
type AdminCurriculum = Awaited<
  ReturnType<typeof adminListCurricula>
>["curricula"][number];
...
function CurriculaTab() {
  ...
  async function load(reset: boolean) {
    reset ? setLoading(true) : setLoadingMore(true);
    try {
      const result = await adminListCurricula({
        data: { offset: reset ? 0 : curricula.length, limit: 25, query },
      });
      setCurricula((prev) =>
        reset ? result.curricula : [...prev, ...result.curricula],
      );
      setHasMore(result.hasMore);
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Couldn't load curricula");
    } finally {
      setLoading(false);
      setLoadingMore(false);
    }
  ...
  async function handleDelete(id: string) {
    ...
    setDeletingId(id);
    try {
      await adminDeleteCurriculum({ data: { id } });
      setCurricula((prev) => prev.filter((c) => c.id !== id));
      toast.success("Curriculum deleted");
    } catch (e) {
      toast.error(
        e instanceof Error ? e.message : "Couldn't delete curriculum",
      );
    } finally {
      setDeletingId(null);
      setConfirmId(null);
  ...
  return (
    <div>
      <SearchBar
        value={query}
        onChange={setQuery}
        placeholder="Search by program name or owner email…"
      />
      {loading ? (
        <Loader2 className="h-5 w-5 animate-spin text-primary" />
      ) : curricula.length === 0 ? (
        <p className="text-sm text-muted-foreground">No curricula found.</p>
      ) : (
        <div className="rounded-xl border bg-card divide-y">
          {curricula.map((c) => (
            <div key={c.id} className="flex items-center gap-3 p-3">
              <div className="min-w-0 flex-1">
                <p className="text-sm font-medium truncate">
                  {c.programName || "Untitled program"}
                </p>
                <p className="text-xs text-muted-foreground truncate">
                  {c.sourceFilename || "—"} • {c.ownerEmail || "no owner"} •
    ...
    <div className="space-y-6">
      <div>
        <h2 className="flex items-center gap-1.5 text-sm font-semibold mb-3">
          <KeyRound className="h-4 w-4 text-primary" /> Groq API key
        </h2>
        <div className="rounded-xl border bg-card divide-y">
          <ConfigRow
            ...
            label={
              status.sharedApiKey
                ...
                : "No shared API key configured"
            }
            hint={
              status.sharedApiKey
                ? "Set via GROQ_API_KEY — anyone using this app without pasting their own key uses this one automatically."
                : "Set GROQ_API_KEY in Vercel's Environment Variables so people don't each need their own key. Until then, everyone must paste their own (get one free at console.groq.com/keys)."
            }
          />
          <ConfigRow
            ...
            label={
              status.deepseekFallback
                ? "DeepSeek fallback is configured"
                : "No fallback provider configured"
            }
            hint={
              status.deepseekFallback
                ? "Set via DEEPSEEK_API_KEY — automatically tried if Groq is rate-limited or erroring on both models."
                : "Set DEEPSEEK_API_KEY so generation doesn't just fail outright if Groq is down or rate-limited."
            }
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="rounded-xl border bg-card divide-y">
          <ConfigRow
            ...
            hint={
              status.database
                ? "DATABASE_URL is set — decks, notes, and accounts persist."
                : "Set DATABASE_URL — without it, nothing is saved."
            }
          />
          <ConfigRow
            ok={status.redis}
            ...
            hint={
              status.redis
                ? "Distributed rate limiting, caching, and locking are active."
                : "Set UPSTASH_REDIS_REST_URL/TOKEN for cross-instance rate limiting."
            }
          />
          <ConfigRow
            ok={status.session}
            ...
            hint={
              status.session
                ? "SESSION_SECRET is set — sign-in is available."
                : "Set SESSION_SECRET (32+ chars) to enable accounts."
            }
          />
          <ConfigRow
            ...
            hint={
              status.googleAuth
                ? "Configured."
                : "Set VITE_GOOGLE_CLIENT_ID + GOOGLE_CLIENT_ID to enable."
            }
          />
          <ConfigRow
            ...
            hint={
              status.passwordAuth
                ? "Available."
                : "Needs sessions + database configured."
            }
          />
          <ConfigRow
            ok={status.email}
            ...
            hint={
              status.email
                ? "Password-reset emails are actually delivered."
                : "Set RESEND_API_KEY — until then, reset links are only logged server-side."
            }
          />
        </div>
      </div>
      ...
      <div>
        ...
        <div className="rounded-xl border bg-card divide-y">
          <ConfigRow
            ...
            label={
              status.r2Storage
                ? "R2 file storage is configured"
                : "R2 file storage not configured"
            }
            hint={
              status.r2Storage
...
function BackupNowRow() {
  ...
  async function handleBackup() {
    ...
    try {
      ...
      toast.success(
        `Backup saved (${result.counts.decks} decks, ${result.counts.curricula} curricula)`,
      );
    ...
    <div className="flex items-center gap-3 p-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-xs text-muted-foreground">
          {lastResult ?? "Dumps all data to your configured backup storage."}
        </p>
      </div>
      <button
        type="button"
