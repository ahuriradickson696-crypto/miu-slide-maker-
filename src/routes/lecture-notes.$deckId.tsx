...
import { generateLectureNotes, getLectureNotes, type LectureNotes } from "@/lib/lecture-notes.functions";
...
function LectureNotesPageInner() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setLoadError(null);
    Promise.all([getDeck({ data: { id: deckId } }), getLectureNotes({ data: { deckId } })])
      .then(([d, n]) => {
        if (cancelled) return;
        setDeck(d);
        setNotes(n);
      ...
      .catch((e) => {
        if (cancelled) return;
        setLoadError(e instanceof Error ? e.message : "Couldn't load this deck.");
      })
      .finally(() => {
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      const message = e instanceof Error ? e.message : "Couldn't generate lecture notes.";
  ...
  const toc = useMemo(() => {
    ...
    const items = [
      ...
      ...notes.sections.map((s, i) => ({ id: slugify(s.heading, i), label: `${i + 1}. ${s.heading}` })),
      { id: "takeaways", label: "Key Takeaways" },
    ];
    if (notes.furtherReading.length) items.push({ id: "reading", label: "Further Reading" });
    return items;
  }, [notes]);
    ...
    <div className="min-h-screen bg-[#F7F8F5]">
      ...
      {loadError && !loading && (
        <div className="mx-auto max-w-md px-4 py-24 text-center">
          ...
          <a href="/slides" className="mt-4 inline-block text-sm font-medium text-primary hover:underline">
        ...
        <div className="mx-auto max-w-5xl px-3 sm:px-6 py-6 sm:py-10">
          {!notes ? (
            ...
            >
              ...
              <h1 className="text-lg font-semibold">Generate lecture notes for</h1>
              <p className="mt-1 text-sm font-medium text-primary">{deck.topic}</p>
              <p className="mt-3 text-xs text-muted-foreground leading-relaxed">
                Turns this {deck.slides.length}-slide deck into a formal, MIU-branded lecture-notes document —
                full prose explanations, a glossary of key terms, learning outcomes, and a summary — ready to
                print, hand out, or post for students to study from.
              </p>
              ...
              >
                {generating ? (
                  <>
                    <Loader2 className="h-4 w-4 animate-spin" /> Writing lecture notes…
                  </>
                ) : (
                  <>
                ...
                <div className="miu-gradient px-5 sm:px-10 pt-6 sm:pt-8 pb-6 text-white relative overflow-hidden">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-center gap-3">
                      <img src={logo} alt="" className="h-11 w-11 sm:h-14 sm:w-14 rounded-lg bg-white p-1 shrink-0" />
                      <div className="min-w-0">
                        <p className="text-[10px] sm:text-xs font-bold tracking-wider uppercase truncate">
                          {MIU_FACTS.legalName}
                        </p>
                        <p className="text-[9px] sm:text-[11px] italic opacity-90 truncate">"{MIU_FACTS.motto}"</p>
                      </div>
                    </div>
                  ...
                  <p className="mt-1 text-[10px] sm:text-[11px] opacity-70">
                    Generated {new Date(notes.generatedAt).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}
                  </p>
                </div>
                ...
                <div className="px-5 sm:px-10 py-6 sm:py-8 space-y-8">
                  {notes.overview && (
                    <section id="overview" className="scroll-mt-24">
                      <SectionHeading icon={<Target className="h-4 w-4" />} label="Overview" />
                      <p className="text-sm sm:text-[15px] leading-relaxed text-slate-700">{notes.overview}</p>
                    </section>
                  )}

                  {notes.learningOutcomes.length > 0 && (
                    <section id="outcomes" className="scroll-mt-24">
                      <SectionHeading icon={<ListChecks className="h-4 w-4" />} label="Learning Outcomes" />
                  ...
                  {notes.sections.map((section, i) => (
                    <section key={i} id={slugify(section.heading, i)} className="scroll-mt-24">
                      <h2
                      ...
                      <div className="space-y-3">
                        {section.paragraphs.map((p, pi) => (
                          <p key={pi} className="text-sm sm:text-[15px] leading-relaxed text-slate-700">
                            {p}
                          </p>
                        ))}
                      </div>
                          ...
                          <dl className="space-y-1.5">
                            {section.keyTerms.map((t, ti) => (
                              <div key={ti} className="text-xs sm:text-sm">
                                <dt className="inline font-semibold text-slate-800">{t.term}</dt>
                                <dd className="inline text-slate-600"> — {t.definition}</dd>
                              </div>
                            ))}
                          </dl>
                        </div>
                  ...
                  {notes.keyTakeaways.length > 0 && (
                    <section id="takeaways" className="scroll-mt-24">
                      <div className="rounded-xl bg-primary/5 border border-primary/20 p-4 sm:p-5">
                        <SectionHeading icon={<Sparkles className="h-4 w-4" />} label="Key Takeaways" noBorder />
                  ...
                  {notes.furtherReading.length > 0 && (
                    <section id="reading" className="scroll-mt-24">
                      <SectionHeading icon={<Library className="h-4 w-4" />} label="Further Reading" />
