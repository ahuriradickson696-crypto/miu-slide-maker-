...
export function ensureSchema(): Promise<void> {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      `;
      // Short-lived cache of full AI provider responses, keyed by a hash of the
      ...
      `;
      // Notes Engine decoupling — same reasoning as the quizzes migration
      // above: NULL deck_id values never collide with the existing UNIQUE
      // constraint, so standalone notes and the original one-per-deck rule
      // coexist without further schema changes.
      await db`ALTER TABLE lecture_notes ALTER COLUMN deck_id DROP NOT NULL`;
      ...
      // A snapshot of a deck's slides at a point in time. Created
      // explicitly (a "Save version" action), not on every micro-edit —
      // keeps this table meaningful instead of noisy, and avoids a write
      // on every keystroke. The live `slides` table always reflects the
      // most recent version; this table is purely historical/restorable.
      await db`
      ...
      // ========== Shareable links ==========
      // A single table covers every shareable resource type (deck,
      // lecture notes, curriculum semester) rather than one table per
      // type — a share is just a pointer (resource_type + resource_id)
      // plus its own access token and optional expiry, so adding a new
      // shareable resource type later needs no schema change here.
      await db`
      ...
      // ========== Quizzes (its own dedicated module) ==========
      // One row per deck (regenerating overwrites, same pattern as
      // lecture_notes) — questions stored as JSONB since their shape
      // (MCQ options vs short-answer) varies per question.
      await db`
      ...
      `;
      // Quiz Engine decoupling: a quiz no longer has to belong to a deck.
      // deck_id's existing UNIQUE constraint is untouched and still works
      // correctly once nullable — Postgres never treats two NULLs as
      // duplicates, so any number of standalone quizzes (deck_id IS NULL)
      // can coexist alongside the original one-quiz-per-deck rule.
      await db`ALTER TABLE quizzes ALTER COLUMN deck_id DROP NOT NULL`;
    })();
  }
  return schemaReady;
}
