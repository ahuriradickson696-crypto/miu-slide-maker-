...
//   this halves your request count against the free-tier rate limit. Slide
//   content AND per-slide speaker notes come out of this SAME single call
//   (see deckSchema.speakerNotes) — there is no separate "notes" request,
//   so slides and notes can never drift out of sync with each other.
...
export type SlideSpec = {
  ...
  // Detailed lecturer/speaker notes for this exact slide — generated in the
  // SAME Gemini call as the slide content itself (never a second request).
  // This is where the "extra depth" lives: full explanations, examples,
  // context, and talking points that don't fit on the visual slide.
  speakerNotes?: string;
};
...
// On-slide content is a bit richer than before (more bullets, longer body),
// while the bulk of the "give me more information" ask is satisfied by
// speakerNotes below — full paragraphs of depth that live in the notes pane
// instead of being crammed onto the visible slide.
const MAX_BULLETS = 6;
const MAX_BULLET_CHARS = 170;
const MAX_BODY_CHARS = 420;
const MAX_TITLE_CHARS = 50;
const MAX_SPEAKER_NOTES_CHARS = 1600;
...
function clampSlide(spec: Record<string, unknown>): SlideSpec {
  ...
  return {
    ...
    speakerNotes:
      typeof spec.speakerNotes === "string" && spec.speakerNotes.trim()
        ? clamp(spec.speakerNotes, MAX_SPEAKER_NOTES_CHARS)
        : undefined,
  };
}
...
- For deep-explanation slides: combine a "body" paragraph (max ${MAX_BODY_CHARS} chars) explaining the "why/how" with supporting "bullets" (max ${MAX_BULLETS}, ${MAX_BULLET_CHARS} chars each). Use the full space available — don't leave a slide thin when there's more to teach.
...
- The visible slide itself must stay sparse, professional, and free of dense paragraphs — it's a visual aid, not the lecture.

SPEAKER NOTES — REQUIRED ON EVERY SLIDE, GENERATED RIGHT NOW IN THIS SAME RESPONSE:
- This is where ALL the extra depth goes. Every slide (including title, identification, and takeaway) must include a "speakerNotes" field: a substantial, well-written passage (aim for 120-220 words on content/list slides, shorter for title/identification) written as if you are the lecturer talking through the slide.
- Speaker notes must go far beyond restating the bullets: give full explanations, worked examples, real-world applications, common misconceptions, definitions of key terms, and transition/context sentences linking to the previous and next slide.
- Do NOT produce slides first and notes later, and do NOT summarize the slide back to itself — write notes as if teaching a student who can only hear you, not read the slide. Slide content and speaker notes must be generated together, in this one pass, so they stay perfectly in sync with each other.
  ...
  try {
    const res = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: {
          temperature: 0.7,
          // Raised from 8192 — slides now carry more on-slide content plus a
          // full speakerNotes passage per slide, generated in this one call.
