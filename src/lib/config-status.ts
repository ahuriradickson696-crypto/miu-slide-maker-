...
// Nothing here is required to run the app — GROQ_API_KEY is supplied
...
export function getConfigStatus(): ConfigStatus {
  ...
  const sharedApiKey = !!process.env.GROQ_API_KEY;
...
export function logStartupConfigStatus() {
  ...
  if (!status.database) {
    notes.push(
      "DATABASE_URL not set — deck History will be unavailable (generation/download still work).",
    );
  }
  if (!status.redis) {
    notes.push(
  ...
  if (status.session && !process.env.GOOGLE_CLIENT_ID) {
    notes.push(
      "GOOGLE_CLIENT_ID not set — Google Sign-In will be unavailable even though SESSION_SECRET is configured.",
    );
  }
  ...
  if (!status.adminDashboard) {
    notes.push(
      "ADMIN_EMAILS not set (or DATABASE_URL missing) — the /admin usage dashboard will be unavailable.",
    );
  }
  if (!status.sharedApiKey) {
    notes.push(
      "GROQ_API_KEY not set — no shared server-side key; each person will need to paste their own Groq API key to generate content.",
    );
  }
  if (!status.deepseekFallback) {
    notes.push(
      "DEEPSEEK_API_KEY not set — no fallback provider if Groq is rate-limited or down; generation will just fail in that case.",
    );
  }
  if (!status.r2Storage) {
    notes.push(
